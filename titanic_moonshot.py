...
# --- Preprosesserings-pipelines ---
num_pipe = Pipeline([("imp", SimpleImputer(strategy="median"))])

# Viktig: bruk tetthets-output (sparse=False). Dette virker i alle sklearn-versjoner.
cat_pipe = Pipeline([
    ("imp", SimpleImputer(strategy="most_frequent")),
    ("oh", OneHotEncoder(handle_unknown="ignore", sparse=False))
])

# ColumnTransformer – tving også tett kombinasjon
pre = ColumnTransformer(
    [("num", num_pipe, num_cols), ("cat", cat_pipe, cat_cols)],
    sparse_threshold=0.0
)

# Ekstra sikkerhet: konverter til tett hvis noe likevel er sparse
...
pipe = Pipeline([("pre", pre), ("to_dense", to_dense), ("clf", ensemble)])
...
pd.DataFrame({"PassengerId": test_df["PassengerId"], "Survived": test_pred}).to_csv(BASE / "submission.csv", index=False)
